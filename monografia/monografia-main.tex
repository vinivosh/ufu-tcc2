...
A função \textit{\pythoninline{kMeansGPU}} acima é a função principal dessa implementação do k-means, que por sua vez chama outras três onde o processamento mais intenso é realizado paralelamente pela GPU. Essas outras funções são discutidas mais adiante (algoritmos \ref{code:kMeansGPU.calcDistances}, \ref{code:kMeansGPU.calcClosestCentroids} e \ref{code:kMeansGPU.calcLogs}). A estrutura do código não muda muito em relação ao Algoritmo \ref{code:kMeansCPU}, então apenas as partes mais modificadas serão explicadas aqui.
...
Por este motivo, são feitas as declarações de duas variáveis importantes: \textit{\pythoninline{n}} na linha 2, armazenando a quantidade de instâncias do dataset, e \textit{\pythoninline{d}}, armazenando a quantidade de variáveis que compõe cada datapoint. Ambas informações são inferidas a partir do dataframe passado à função. Além disso, são feitas também as conversões de objetos dataframe para arrays do \textit{Numpy} nas linhas 8--11.
...
Na linha 14 é chamada a primeira função paralelizada, a \textit{\pythoninline{calcLogs}} (Algoritmo \ref{code:kMeansGPU.calcLogs}). Esse é o passo de cálculo dos logarítimos de todos os datapoints, equivalente à operação realizada na linha 5 do Algoritmo \ref{code:kMeansCPU}.
...
A inicialização de mais um array é feita na linha 28, dessa vez \textit{\pythoninline{meansByClosestCent}}, de dimensão $n \cdot d$, que irá armazenar, para cada centróide, as médias das coordenadas de todos os datapoints --- ou dos logarítimos das delas, mais precisamente --- mais próximos dele.

Na linha 30 é iniciado um outro laço de repetição que realiza, para cada centróide, o cálculo do novo centróide respectivo que será usado na próxima iteração. Tal operação é realizada de maneira serial. Primeiro, na linha 31, é criada uma \enquote{máscara} que é usada, na linha 32, para filtrar apenas os logarítimos dos datapoints mais próximos do centróide atualmente sendo calculado, armazenando estes na variável \textit{\pythoninline{relevantLogs}}. Esse laço de repetição inteiro corresponde às operações da linha 15 no Algoritmo \ref{code:kMeansCPU}.

É feita também uma checagem importante na linha 34. É checado se não há nenhum datapoint mais próximo ao centróide atual, um cenário que pode ocorrer em execuções do k-means. Quando isso ocorre, o cálculo do novo centróide atual é simplesmente pulado. Isso causa esse centróide a se manter o mesmo na próxima iteração, o que é o esperado quando não há nenhuma informação para cálculo de sua nova posição. Sem essa checagem, um \textit{warning} do \textit{Numpy} ocorreria mais adiante, no cálculo da média dos datapoints.

Na linha 35 é finalmente realizado o cálculo da média dos logarítimos dos datapoints mais próximos ao centróide atual, usando a função \textit{\pythoninline{mean}} do \textit{Numpy}. O valor é salvo na variável \textit{\pythoninline{meansByClosestCent}}, no índice correspondente ao centróide atualmente sendo analisado.
...
As linhas 40 e 41 incluem apenas a marcação da variável \textit{\pythoninline{meansByClosestCent}} para exclusão e o acréscimo em um da variável de controle \textit{\pythoninline{iteration}}. O laço de repetição das iterações do k-means é então fechado, e na última linha o resultado do agrupamento é devolvido, na variável \textit{\pythoninline{closestCent}}.

Os Algoritmos \ref{code:kMeansGPU.calcLogs}, \ref{code:kMeansGPU.calcDistances} e \ref{code:kMeansGPU.calcClosestCentroids} abaixo mostram as funções vetoriais que rodam na GPU NVIDIA, onde os cálculos mais pesados são realizados. Como discutido mais a fundo no capítulo \ref{sec:numba}, funções como essas, implementadas com a biblioteca\textit{Numba} para rodar em CUDA, trabalham com um elemento de cada vez, sendo chamadas milhares de vezes e rodando paralelamente, cada chamada em um núcleo (\textit{CUDA core}) da GPU, fazendo o cálculo apenas de seu respectivo elemento ou sub-conjunto da estrutura de dados sendo processada, atingindo altíssimos níveis de paralelição. Estas funções também não podem retornar valores diretamente, necessitando ao invés de uma variável de retorno, passada como argumento. Essa variável foi definida aqui sempre como o último argumento da função.

\pythonexternal[
  firstline=3,
  lastline=8,
  label=code:kMeansGPU.calcLogs,
  caption={Implementação paralela do cálculo dos logaritmos dos datapoints},
]{code-snippets/kMeansGPU.py}
...
Ela recebe um datapoint apenas (\textit{\pythoninline{rowDataset}}) e, na linha 6, calcula o logarítimo natural de cada variável que compõe o datapoint, salvando o resultado no array de retorno (\textit{\pythoninline{rowResults}}). Nas linhas 2 e 3 são declaradas as especificações obrigatórias de tipo e dimensionalidade dos argumentos da função. Ambos são vetores unidimensionais de floats de 64 bits e de tamanho $d$, letra que representa a quantidade de dimensões dos datapoints.
...
\textbf{TODO: Escrever parágrafos explicando a função calcDistances().}
...
\textbf{TODO: Escrever parágrafos explicando a função calcClosestCentroids().}
