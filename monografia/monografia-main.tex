...
Para exemplificar bem o processo de paralelização de um algoritmo utilizando CUDA, é apresentada aqui uma implementação simples, na linguagem C++, de um programa que soma dois vetores unidimensionais com mais de 250 milhões de elementos cada, do tipo ponto flutuante de precisão simples (\textit{float}) --- o que ocupa cerca de 2 GB de memória no total. É importante que a máquina onde o algoritmo é executado possuir essa quantidade de memória RAM disponível, assim como VRAM na GPU. Mais detalhes sobre a máquina utilizada para testes no Capítulo \ref{sec:máquinaUtilizada}.
...
Rodando a função \textit{add()} e medindo seu tempo de execução, encontramos que o tempo médio entre dez execuções é de cerca de 313 milisegundos.

\textbf{TODO: Escrever parágrafos sobre os outros códigos!}
...
\codeexternal[
  ...
  caption={Implementação paralela ingênua de soma de vetores usando CUDA},
]{code-snippets/addArrayGPUv2.cu}

\textbf{TODO: Explicar o código do addArrayGPUv2. Tempo de execução média em 10 execuções do: ~410 ms.}

\codeexternal[
  language=C++,
  % firstline=42,
  % lastline=,
  label=code:addArrayGPUv3.cpp,
  caption={Implementação paralela de soma de vetores usando CUDA},
]{code-snippets/addArrayGPUv3.cu}

\textbf{TODO: Explicar o código do addArrayGPUv3. Tempo de execução média em 10 execuções do: ~44 ms.}
