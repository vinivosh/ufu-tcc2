...
Note que há duas importações importantes omitidas nos códigos exibidos neste capítulo, por motivos de brevidade. Uma é a da biblioteca Numpy, importada com o nome \textit{np}; a outra é a biblioteca Pandas, importada com o nome \textit{pd}.
...
Entramos então num laço de repetição (\textit{while loop}) na linha 7, onde o agrupamento dos dados é de fato realizado, iterativamente. Todo o restante do algoritmo é realizado dentro deste loop, exceto o retorno do resultado de classificação no final. A condição de parada é testada aqui, que é (1) a iteração atual, ainda a ser realizada, ser maior que \textit{\pythoninline{maxIter}}, o número de iterações máximas ou (2) os centróides recém calculados serem exatamente iguais aos centróides calculados na iteração anterior --- o que significa que o agrupamento atual é o melhor que o algoritmo pode atingir nessa execução, ou seja, um máximo local. Como explicado no capítulo \ref{sec:kmeans}, o k-means não garante que os agrupamentos convirjam para um máximo global, apenas um máximo local.
...
\textbf{TODO: Escrever mais parágrafos entre estes, explicando todo o código.}
