...
O K-means é um algoritmo notório de agrupamento de dados que vem sendo usado extensivamente desde sua concepção no meio do século XX. Utilizando-o como exemplo de implementação, essa pesquisa visou implementar uma versão paralelizada, usando o processamento vetorial de uma GPU NVIDIA, para obter um ganho de performance significativo, sem afetar a precisão dos resultados. Essa implementação foi feita utilizando ferramentas de programação de mais alto-nível, especificamente a linguagem Python e bibliotecas como \textit{Numpy} e \textit{Numba}, explorando a possibilidade de se tingir grandes aumentos de velocidade de execução sem abrir mão da facilidade de desenvolvimento proporcionada pelo Python. Ganhos médios de peformance entre 9x a 42x foram atingidos com sucesso sem prejudicar mensuravelmente a qualidade dos resultados, ao se comparar uma implementação serial usando \textit{Pandas} e \textit{Numpy} e uma paralela usando, além dessas, a biblioteca \textit{Numba} para rodar \textit{kernels} CUDA na GPU.
