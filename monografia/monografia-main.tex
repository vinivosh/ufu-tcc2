...
Um dos motivos da fama do k-means é a simplicidade de sua implementação original. Ela pode ser descrita com um pseudocódigo extremamente sucinto, como o exibido abaixo.
...
O Algoritmo \ref{code:kMeans.pseudo} acima descreve a implementação, como originalmente proposta, do algoritmo k-means. Não há garantia de que o processo convirja para um máximo global, apenas para um máximo local que, dependendo dos centróides iniciais escolhidos, pode ser arbitrariamente pior que o máximo global.

A complexidade de tempo do k-means é de $O(N K D I)$ \cite{surveyClustAlgoXuWunsch2005}, onde $I$ é o número de iterações necessárias para ele converger em um resultado, estando na faixa de $[1, I_{max}]$ e dependendo dos centróides iniciais escolhidos.
