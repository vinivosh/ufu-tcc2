...
% Definindo float environment para posicionar equações com legendas no documento

\usepackage{float}
\usepackage{aliascnt}
\newaliascnt{eqfloat}{equation}
\newfloat{eqfloat}{h}{eqflts}
\floatname{eqfloat}{Equação}
...
\titulo{Paralelização do Algoritmo K-means em GPUs NVIDIA Utilizando a Biblioteca Numba} %TCC
...
\begin{folhadeaprovacao}
  ...
  \begin{center}
   ...
   \assinatura{\textbf{Júlia Tannús de Souza} \\ Convidado 1} %TCC:
   \assinatura{\textbf{Anderson Rodrigues dos Santos} \\ Convidado 2} %TCC:
...
% Tamanho atual desse resumo: 296 palavras e 1.939 caracteres
...
Além disso, todo o código utilizado no trabalho, desde as implementações de algoritmos testados, até o arquivo \textit{LaTeX} da monografia, estão disponíveis no repositório do GitHub em: \href{https://github.com/vinivosh/ufu-tcc2}{https://github.com/vinivosh/ufu-tcc2}.
...
\begin{figure}[h]
  \caption{Saída de algoritmos de agrupamento de dados ao processarem conjuntos de dados 2D variados}
  \centering
  \includegraphics[width=0.95\textwidth]{clusteringExamplesScikitLearn.png}
  \label{fig:clusteringExamples}
\end{figure}

Essa diversidade é exemplificada na Figura \ref{fig:clusteringExamples} \cite{clusteringExamplesSciKitLearn}, retirada da documentação da biblioteca \textbf{SciKit Learn} para a linguagem Python, que oferece diversas ferramentas e algoritmos essenciais para área de ciência de dados e aprendizado de máquina. Na imagem, são exibidas as saídas de oito algoritmos de agrupamento de dados, ao processarem quatro conjuntos de dados compostos por objetos com duas \textit{features} (variáveis). Essas saídas são representadas graficamente distribuindo os pontos de dados em dois eixos, cada um representando uma variável que descreve o ponto. A cor dos pontos representa a qual grupo cada objeto de dado pertence. Há também o tempo de execução, em segundos, no canto inferior direito de cada saída.

O algoritmo \textit{Mini-Batch K-Means}, por exemplo, não foi capaz de dividir adequadamente o primeiro conjunto de dados, com dois grupos no formato de círculos circunscritos, enquanto o \textit{DBSCAN} e \textit{Agglomerative Clustering} conseguiram diferenciar os dois grupos sem problemas. O mesmo cenário ocorreu com o segundo conjunto de dados, com datapoints distribuídos em segmentos de elipses. É possível notar também que os algoritmos \textit{Affinity Propagation} e \textit{Spectral Clustering} possuem tempos de execução significativamente maiores que os outros seis algoritmos.
...
A implementação de versões paralelas de outros algoritmos de agrupamento de dados envolveria um processo de identificação de partes de código paralelizável diferente para cada algoritmo, mesmo que semelhante ao realizado aqui para o k-means --- processo apresentado no capítulo \ref{chp:identifPotencParalel}. Algumas dificuldades peculiares também poderiam se apresentar para cada algoritmo a ser paralelizado, como operações de redução, onde a entrada é vetorial e a saída é escalar, que são mais difíceis de se paralelizar do que operações onde ambas a entrada e saída são vetoriais. Um exemplo de operação como essa existe no próprio algoritmo k-means, e é comentada mais a fundo no capítulo \ref{sec:kMeansImplementações}.
...
Na Figura \ref{fig:kMeansDemoIris} mais abaixo há um exemplo de uma execução do k-means, utilizando o dataset Iris, explicado em mais detalhes no Capítulo \ref{sec:datasetsUtilizados}.
...
\subsection{História}
\label{ssc:historiaProgVetorial}
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \caption{Cubo antes de rotações}
    \includegraphics[width=0.9\textwidth]{defaultCube.png}
    \label{fig:cubeBefore}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \caption{Cubo após rotações}
    \includegraphics[width=0.9\textwidth]{defaultCubeRotated.png}
    \label{fig:cubeAfter}
  \end{minipage}
\end{figure}

Com o surgimento das GPUs, desenvolvidas com o propósito inicial de acelerar a renderização gráfica em jogos e aplicações de multimídia, surgiu uma nova oportunidade para a programação vetorial. As GPUs são compostas por centenas ou até milhares de núcleos de processamento, o que as torna altamente paralelizáveis e adequadas para executar operações vetoriais em larga escala. Isso possibilitou a utilização das GPUs não apenas para gráficos, mas também para tarefas de computação de propósito geral (área chamada também de \textit{GPGPU}), incluindo processamento de grandes conjuntos de dados e algoritmos de aprendizado de máquina.

Um exemplo prático de operações gráficas comumente realizadas por GPUs é exemplificado nas figuras \ref{fig:cubeBefore} e \ref{fig:cubeAfter}, que exibem, respectivamente, um cubo no espaço tridimensional antes e depois de sofrer duas rotações --- uma de 60 graus no sentido horário no eixo $x$ e outra de 30 graus no sentido anti-horário no eixo $y$, seguindo a convenção da \textit{Regra de Fleming}, popularmente conhecida como \enquote{\textit{regra da mão direita}}.

Essa operação de rotação pode parecer simples visualmente, mas internamente requer duas multiplicações de matrizes com valores obtidos através das operações trigonométricas de seno e cosseno. Essas multiplicações de matrizes, exibidas nas equações \ref{eq:rotationX60CW} e \ref{eq:rotationY30CCW}, precisam ser realizadas para cada vértice de todos os polígonos visíveis na cena a ser renderizada pela placa de vídeo.
...
\begin{eqfloat}[h]
  \begin{equation}
    ...
    \end{array} } \right]
  \end{equation}
  \caption{Operação para rotacionar pontos 60 graus no sentido horário no eixo $x$}
  \label{eq:rotationX60CW}
\end{eqfloat}
...
Rodando o programa e medindo o tempo de execução da função \textit{add}, é encontrado que o tempo médio entre cem execuções consecutivas é de cerca de \textbf{462 milissegundos} e o erro máximo é zero.

% ? Realmente vale a pena mostrar essa versão coitada que nem paralela na verdade é? Acho que não… a performance disso decai na verdade, em relação à versão CPU, então nem faz sentido comparar isto com nada, sinceramente.
...
\codeexternal[
  ...
  firstline=4,
  lastline=10,
  caption={Implementação vetorial de soma de vetores usando CUDA --- Trecho 2},
...
Rodando o algoritmo e medindo o tempo cada execução, obtem-se o tempo médio de cerca de \textbf{17 milissegundos}. O erro máximo continua em zero. Houve agora um \textit{speed-up} de cerca de 8,88 vezes em relação à versão vetorial anterior (Algoritmo \ref{code:addArrayGPUv2.cpp}) e de cerca de 27,17 vezes em relação à versão serial rodando em CPU (Algoritmo \ref{code:addArrayCPU.cpp}).

\begin{table}[h]
  \centering
  \caption{Comparação de performance da soma de vetores (com $2^{28} + 2^{27}$ elementos) em C++. \textit{Speed-up} calculado em relação à versão serial (CPU)}
  \label{tab:sumArrCPP}
  \begin{tabular}{|l|l|l|}
    \hline
    \rowcolor[HTML]{9B9B9B} 
    \textbf{Implementação} & \textbf{Tempo de Exec. (ms)} & \textbf{Speed-up} \\ \hline
    CPU (Algoritmo \ref{code:addArrayCPU.cpp}) & 462 & --- \\ \hline
    GPU (ingênua) (Algoritmo \ref{code:addArrayGPUv2.cpp}) & 151 & 3,05x \\ \hline
    GPU (Algoritmo \ref{code:addArrayGPUv3.snippet1.cpp} e \ref{code:addArrayGPUv3.snippet2.cpp}) & 17 & 27,17x \\ \hline
  \end{tabular}
\end{table}
...
Para exemplificar como foi feito o processo, um algoritmo simples, equivalente ao usado para demonstrar o uso da API CUDA em C++ no Capítulo \ref{ssc:implementaçãoCUDA}, foi implementado, primeiro serialmente, e depois paralelamente, utilizando a GPU.

O Algoritmo \ref{code:addArrayCPU.py} abaixo executa a adição de dois vetores, retornando o resultado num terceiro vetor novo. Note que foram importadas, em ambas versões do algoritmo, as bibliotecas \textit{Numba} e \textit{Numpy}, importada com o nome \textit{np}. Os comandos de \textit{\codeinline{import}} foram todos omitidos por brevidade. Foram utilizados vetores e funções do \textit{Numpy} pois são mais performáticos do que seus equivalentes em Python puro.

\codeexternal[
  firstline=3,
  lastline=26,
  caption={Implementação serial de adição de vetores em Python},
  label=code:addArrayCPU.py,
]{code-snippets/addArrayCPU.py}

O código acima é auto-explicativo e está comentado para melhor compreensão. Se assemelha bastante com o Algoritmo \ref{code:addArrayCPU.cpp} exibido no Capítulo \ref{ssc:implementaçãoCUDA}. Os vetores continuam com o mesmo tamanho de $N = 402.653.184$. A única diferença prática é que aqui o resultado é retornado em um novo vetor (variável $c$), ao invés de haver uma sobrescrita dos valores do segundo vetor da soma (varíavel $b$).

% As diferenças práticas são o tamanho dos vetores. Aqui, eles possuem a dimensões $N \cdot D$, onde $N = 100.663.296$ e $D = 4$, totalizando $402.653.184$ elementos. Isso são exatamente 50\% a mais elementos do que havia no exemplo em C++.

Rodando o código e realizando a medição da velocidade de execução da função \textit{addArrayCPU}, encontra-se o tempo médio (de cem execuções) de cerca de \textbf{1.624,79 milissegundos}. O erro máximo (calculado pela função \textit{checkMaxErr} na última linha) é de zero.
...
\codeexternal[
  ...
  firstline=26,
  lastline=30,
  caption={Implementação vetorial de adição de vetores em Python --- trecho 1},
...
\codeexternal[
  ...
  firstline=3,
  lastline=8,
  caption={Implementação vetorial de adição de vetores em Python --- trecho 2},
  label=code:addArrayGPU.snippet2.py,
]{code-snippets/addArrayGPU.py}

Com apenas essa modificação a mais o código está pronto para ser executado paralelamente pela GPU. Note que, mesmo a função \textit{addArrayGPU} lidando apenas com valores escalares, eles devem ser acessados como o primeiro elemento de listas (\textit{a[0]} ao invés de apenas \textit{a}). Isso é um requerimento do uso da funcionalidade \textit{guvectorize} do \textit{Numba}.
...
Tudo que é necessário para tal funcionamento extremamente automatizado de paralelização é, primeiro, implementar a função vetorial em Python como uma que recebe e processa apenas um elemento da estrutura de dados maior que de fato está sendo processada, como foi feito na última linha do Algoritmo \ref{code:addArrayGPU.snippet2.py}, e, segundo, informar ao decorador \textit{guvectorize} alguns argumentos a respeito dos dados que a função irá receber e também sobre seu modo de compilação e execução.

São passados quatro argumentos para o decorador. O primeiro é uma lista de um elemento do tipo \textit{string} que define os tipos de variáveis que cada instância da função vai receber, em sua \textit{thread}. Nesse caso, como há três argumentos em \textit{addArrayGPU}, são definidos três tipos dentro dos parênteses após a palavra \textit{void}. Todos os tipos são \textit{float32[:]}, o que indica que são escalares ou vetores unidimensionas contendo números de ponto flutuante (\textit{float}) de precisão simples. Nesse caso, os elementos serão escalares. Se fossem vetores bidimensionais, a sintaxe seria \textit{float32[:,:]}; se tridimensionais, \textit{float32[:,:,:]} e assim por diante.

O segundo argumento do decorador é uma \textit{string} que denota a dimensionalidade dos argumentos que cada instância da função vai receber e processar, em sua \textit{thread}. A sintaxe aqui usada, \textit{(),()->()} indica que há dois valores escalares de entrada e um de saída. Para vetores unidimensionais de tamanho $n$, a sintaxe seria \textit{(n),(n)->(n)}; para bidimensionais de tamanho $n \cdot d$, seria \textit{(n,d),(n,d)->(n,d)} e assim por diante. Importante notar que a dimensionalidade pode diferir entre os argumentos.

Note que ambos o primeiro e segundo argumento do decorador definem o formato de dado que cada instância da função recebe, e não o formato da estrutura de dados inteira a ser processada. Como a função foi definida para receber um valor escalar, quando ela é de fato chamada e um vetor de tamanho $n$ é passado, o compilador do \textit{Numba} infere que devem ser criadas $n$ instâncias da função, e cada uma delas processará apenas um elemento diferente, de valor escalar, paralelizando automaticamente o processo.
...
% 
% CPU:
% Benchmark concluído! Tempo médio: 1624.789522 ± 346.362247 ms
% 
% GPU:
% Benchmark concluído! Tempo médio: 384.859739 ± 24.824842 ms
% 
% Speed-up = 4.2217x

Rodando o algoritmo e medindo, como anteriormente, a velocidade de execução da função \textit{addArrayGPU}, se constata o tempo médio de aproximadamente de \textbf{384,86 milissegundos}. O erro máximo também é zero, como esperado. Foi obtido um \textit{speed-up} de cerca de \textbf{4,22 vezes} em relação à versão serial implementada usando apenas funções do \textit{Numpy} (que rodava, em média, em 1624.79 ms).
...
É importante notar também que ainda há uma penalidade considerável no potencial de ganho de performance, se comparando implementações usando CUDA em C++ e as usando Numba em Python. O ganho de velocidade foi de 27,17 vezes em C++, enquanto em Python foi de apenas 4,22 vezes. Ainda assim, o \textit{speed-up} obtido nas execuções do k-means acelerado usando o Numba foram de magnitudes ainda maiores que o apresentado neste exemplo simples, como explicitado no capítulo \ref{sec:resultados}.

Não se pode deixar de destacar, no entanto, que a versão em C++ da soma de vetores rodando na GPU (Algoritmos \ref{code:addArrayGPUv3.snippet1.cpp} e \ref{code:addArrayGPUv3.snippet2.cpp}) tem um tempo de execução cerca de 22,63 vezes mais rápido que a versão Python rodando em GPU (Algoritmos \ref{code:addArrayGPU.snippet1.py} e \ref{code:addArrayGPU.snippet2.py}) --- 17 ms contra 384,86 ms, respectivamente.
...
É recomendado, em geral, o método de \textit{normalização min-max} para algoritmos como o k-means, que utilizam a distância euclidiana para a construção de grupos \cite{standardizOfVars1988}. Este processo é explicado detalhadamente no capítulo \ref{sec:datasetsUtilizados}.

O \textbf{Algoritmo \ref{code:kMeansCPU}} abaixo é a implementação da versão serial, \textit{single thread}, do algoritmo k-means, usada nos experimentos descritos no capítulo \ref{chp:datasetsEexperimentos}.
...
É importante notar que a operação supracitada do cálculo da média geométrica é realizada aqui utilizando uma operação alternativa (Equação \ref{eq:geoMeanAlt}), mas equivalente, à sua definição comum (Equação \ref{eq:geoMean}).
...
\begin{eqfloat}[h]
  \begin{equation}
    \sqrt[n]{\displaystyle \prod_{i=1}^{n} a_{i}}
  \end{equation}
  \caption{Definição da média geométrica}
  \label{eq:geoMean}
\end{eqfloat}
...
\begin{eqfloat}[h]
  \begin{equation}
    \exp{(\frac{1}{n} \cdot \displaystyle \sum_{i=1}^{n} \ln{a_{i}})}
  \end{equation}
  \caption{Uma definição alternativa equivalente da média geométrica}
  \label{eq:geoMeanAlt}
\end{eqfloat}
...
O \textbf{Algoritmo \ref{code:kMeansGPU}} abaixo é a implementação da versão paralela, \textit{multithread}, do algoritmo k-means, usada nos experimentos (vide capítulo \ref{chp:datasetsEexperimentos}). Note que aqui há mais três novas importações omitidas: os módulos integrados \textit{\codeinline{math}} e \textit{\codeinline{random}}, além da biblioteca \textit{\codeinline{numba}}.
...
% TODO
% TODO
% TODO: Se der tempo, criar uma sub-seção que sumarize algumas das dificuldades e limitações encontradas ao se tentar implementar o k-means GPU com o Numba
% TODO
% TODO
...
\chapter{Datasets e Experimentos}
\label{chp:datasetsEexperimentos}
...
Neste capítulo são explicados os procedimentos e metodologia de todos os experimentos, além de descrições a respeito dos conjutos de dados utilizados nestes. No Capítulo \ref{sec:resultados} são exibidos e discutidos os resultados dos experimentos.
...
\begin{figure}[h]
  ...
  \centering
  \includegraphics[width=0.95\textwidth]{kMeansAvgTimeLog.png}
...
Os fatores de \textit{speed-up} são ainda mais claros no gráfico de barras da Figura \ref{fig:kMeansAvgSpeedup} abaixo. O padrão esperado foi de fato experimentalmente encontrado. O ganho de velocidade fica cada vez melhor com datasets cada vez maiores.
...
% TODO
% TODO
% TODO: Descrever a dificuldade de algumas partes da implementação aqui? Como aquela list comprehension que causou o speedup de 2,67x ao invés de 40x no HHAR
% TODO
% TODO
