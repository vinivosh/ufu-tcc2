...
\begin{table}[h]
  \centering
  \caption{Comparação de performance da soma de vetores (com $2^{28}$ elementos) em C++. \textit{Speed-up} calculado em relação à versão serial (CPU)}
  \label{tab:sumArrCPP}
  \begin{tabular}{|l|l|l|}
    \hline
    \rowcolor[HTML]{9B9B9B} 
    \textbf{Implementação} & \textbf{Tempo de Exec. (ms)} & \textbf{Speed-up} \\ \hline
    CPU (Algoritmo \ref{code:addArrayCPU.cpp}) & 308 & --- \\ \hline
    GPU (ingênua) (Algoritmo \ref{code:addArrayGPUv2.cpp}) & 93 & 3.31x \\ \hline
    GPU (Algoritmo \ref{code:addArrayGPUv3.snippet1.cpp} e \ref{code:addArrayGPUv3.snippet2.cpp}) & 11 & 28x \\ \hline
  \end{tabular}
\end{table}
...
É importante notar também que ainda há uma penalidade considerável no potencial de ganho de performance, se comparando implementações usando CUDA em C++ e as usando Numba em Python. O ganho de velocidade foi de 28 vezes em C++, enquanto em Python foi de apenas 4,22 vezes. Ainda assim, o \textit{speed-up} obtido nas execuções do k-means acelerado usando o Numba foram de magnitudes ainda maiores que o apresentado neste exemplo simples, como explicitado no capítulo \ref{sec:resultados}.
